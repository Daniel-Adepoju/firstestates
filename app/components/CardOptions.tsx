...
const CardOptions = () => {
  ...
  const clientCardOptions = [
    session?.user && session.user.id !== backdrop.selectedData?.agent._id
      ? {
          ...
          link: backdrop.selectedData
            ? `${process.env.NEXT_PUBLIC_BASE_URL!}/chat?recipientId=${backdrop.selectedData?.agent._id}`
            : "",
          className: "",
          icon: <MessageCircle className="w-4 h-4 inline mb-1 mr-2" />,
          function: () => { setBackdrop({ isOptionsOpen: false }) },
        }
      : null,

    {
      ...
      icon: linkCopied ? (
        <ClipboardCheck className="w-4 h-4 inline mb-1 mr-2" />
      ) : (
        <Clipboard className="w-4 h-4 inline mb-1 mr-2" />
      ),
      function: () => {
  ...
  ? null
     : {
          ...
          link: `${process.env.NEXT_PUBLIC_BASE_URL!}/agent-view/${backdrop.selectedData?.agent._id}`,
          className: "cursor-pointer",
    ...
    session?.user && session?.user.id !== backdrop.selectedData?.agent._id
      ? {
          ...
          link: `${process.env.NEXT_PUBLIC_BASE_URL!}/listings/single_listing?id=${backdrop.selectedData?._id}#clientReport`,
